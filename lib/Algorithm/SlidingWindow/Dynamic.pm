...
# Generic, dynamically sized sliding window / deque implemented with a
# circular buffer.
#
# Core operations:
#   push(@items)  - append newest (grows size)
#   shift()       - remove oldest (shrinks size) and return it
#   pop()         - remove newest (shrinks size) and return it
#   slide($item)  - if non-empty: evict oldest + append newest,
#                   size unchanged, return evicted
#                   if empty: behaves like push($item), returns undef
...
# Logical index: 0 is oldest, size-1 is newest.
sub get {
    my ($self, $index) = @_;
...
sub values {
    ...
    my @out = map { $buf->[ ($head + $_) % $cap ] } (0 .. $n - 1);
    return @out;
}
...
sub slide {
    ...
    # overwrite oldest slot with new item; then advance head
    $self->{buf}[$idx] = $item;
...
sub _check_alloc {
    ...
    croak "alloc must be an integer >= 1"
      if $n !~ /\A[1-9]\d*\z/;
    return 1;
}
